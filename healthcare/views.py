...
def patientRecord(request, patientId):
    try:
        ...
        if request.method == 'POST':
                ...
                # Check if pregnancyStatus is "No" and estimatedDelivery is not provided
                if pregnancyStatus == "No":
                    estimatedDelivery = None
                else:
                    # If pregnancyStatus is "Yes", get the estimatedDelivery from the request
                    estimatedDelivery_str = request.POST.get('estimatedDelivery', None)

                    if estimatedDelivery_str is None or estimatedDelivery_str.strip() == '':
                        # Handle case where estimatedDelivery is not provided
                        # You can set it to None or handle it as needed
                        estimatedDelivery = None
                    else:
                        # Convert estimatedDelivery to datetime object
                        try:
                            estimatedDelivery = datetime.strptime(estimatedDelivery_str, '%Y-%m-%d').date()
                        except ValueError:
                            # Handle invalid date format
                            # You can raise an error, return an error response, or handle it as you wish
                            estimatedDelivery = None  # or any other handling mechanism
